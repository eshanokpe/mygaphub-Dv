...
class _MonthlyBudgetState extends State<MonthlyBudget> {
  ...
  void _initializeCurrency() {
    ...
    // Also check calculatorData as fallback (mirrors SavingIncome behaviour)
    if (fullCurrencyString.isEmpty) {
      final calculatorData = providers.calculatorData;
      fullCurrencyString = calculatorData['currency']?.toString() ?? '';
    }
  ...
  Widget build(BuildContext context) {
    ...
    final providerCurrency = providers.currencySymbol ?? '';

    if (providerCurrency.isNotEmpty &&
        selectedCurrency != _extractCurrencySymbol(providerCurrency)) {
  ...
  void _handleContinue() {
    ...
    final budgetData = {
      "currency": context.read<Providers>().currencySymbol ?? selectedCurrency,
    ...
    Calculatormodel parameters = Calculatormodel(
      ...
      extraSave:
          context.read<Providers>().calculatorData['extra_save']?.toString() ??
          '0',
      otherIncome:
          context
              .read<Providers>()
    ...
    try {
      ...
      context.read<Providers>().updateBudgetData(budgetData);

      _submitBudget(budgetData);
