...
import 'Expenaddnote.dart';
import 'expenditure_allocation_summary.dart';
import 'swipeTransaction.dart';

class ViewDetailExpenditureAllocation extends StatefulWidget {
  var item;
  int index;
  ...
  @override
  State<ViewDetailExpenditureAllocation> createState() =>
      _ViewDetailExpenditureAllocationState();
}

class _ViewDetailExpenditureAllocationState
    extends State<ViewDetailExpenditureAllocation> {
      ...
      appBar: AppBar(
        ...
        actions: [
          Container(
            child:
                (totalleft <
                    (widget.item[widget.index]["amount"] != null
                        ? num.tryParse(
                                widget.item[widget.index]["amount"].toString(),
                              ) ??
                              0
                        : 0))
                ? Container()
                                        ...
                                        child: ElevatedButton(
                                          ...
                                          onPressed: () async {
                                            ...
                                            if (response.statusCode == 400) {
                                              ...
                                              if (expenditureresponse
                                                      .statusCode ==
                                                  200) {
                                                var body = jsonDecode(
                                                  expenditureresponse.body,
                                                );
                                                ...
                                                Fluttertoast.showToast(
                                                  ...
                                                  textColor: Colors.white,
                                                  msg:
                                                      'Expenditure Allocation has been Deleted Successfully',
                                                  toastLength:
                                                      Toast.LENGTH_SHORT,
                                                  gravity: ToastGravity.BOTTOM,
                                                );
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Align(
                              alignment: Alignment.topCenter,
                              child: Text(
                                "left of $currency${widget.item[widget.index]["amount"]}",
                                style: TextStyle(
                                  color: Colors.black,
                        ...
                        Row(
                          children: [
                            _showTextField == false
                                ? Text(
                                    "$currency${(double.tryParse(widget.item[widget.index]["amount"]) ?? 0).toStringAsFixed(2)}"
                                        .replaceAllMapped(
                                          RegExp(
                                            r'(\d{1,3})(?=(\d{3})+(?!\d))',
                                          ),
                            ...
                            showDialog(
                              ...
                              builder: (BuildContext context) {
                                return MyDialogBox(
                                  ...
                                  amount: widget.item[widget.index]["amount"],
                                  note: widget.item[widget.index]["note"],
                                  recuring:
                                      widget.item[widget.index]["recuring"],
                                  label: widget.item[widget.index]["label"],
                                );
                              },
                            );
                          },
                          ...
                          child: TextButton(
                            onPressed: () {
                              Navigator.pushReplacement(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => ExpenditureAddNote(
                                    item: widget.item,
                                    index: widget.index,
                                  ),
                                ),
...
class _MyDialogBoxState extends State<MyDialogBox> {
  ...
  void initState() {
    super.initState();
    bool isRecurring = widget.recuring == 1 ? true : false;
    ...
    // Initialize _controller inside initState()
    _controller = ValueNotifier<bool>(isRecurring);
  ...
  updateExpenditureAllocation() async {
    ...
    if (response.statusCode == 200) {
      if (response2.statusCode == 200) {
        ...
        //  var dataExpen = savall["data"]['budget_allocations'];
        var dataExpen = savall["data"]['budget_expenditures'];
        print(dataExpen);
        List res = dataExpen;
        ...
        if (response.statusCode == 200) {
          ...
          setState(() {
            _dataExpen = res
                .map((dataExpen) => SavingAllexpenditure.fromJson(dataExpen))
                .toList();
          });
          timer.cancel();
          Navigator.pushReplacement(
          ...
          EasyLoading.dismiss();
          Fluttertoast.showToast(
            backgroundColor: const Color(0xffD13B56),
            msg: 'Expenditure Allocation has been updated',
            toastLength: Toast.LENGTH_SHORT,
            gravity: ToastGravity.BOTTOM,
          );
          timer.cancel();
