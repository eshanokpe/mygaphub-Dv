...
class _MoreState extends State<More> {
  ...
  iLabData() async {
    ...
    try {
      ...
      if (response2.statusCode == 200) {
        final responseData = response2.data;
        ...
        if (responseData is Map) {
          if (responseData['status'] == true ||
            ...
            Fluttertoast.showToast(
              backgroundColor: const Color(0xff00B050),
              msg: responseData['message'] ?? 'iLab target has been set',
              toastLength: Toast.LENGTH_SHORT,
              gravity: ToastGravity.BOTTOM,
            );
            ...
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const I360LabScreen()),
            );
          } else {
            Fluttertoast.showToast(
              backgroundColor: Colors.red,
              msg: responseData['message'] ?? 'Failed to set iLab target',
              toastLength: Toast.LENGTH_SHORT,
              gravity: ToastGravity.BOTTOM,
            );
          }
        ...
        Fluttertoast.showToast(
          backgroundColor: Colors.red,
          msg: 'Failed with status: ${response2.statusCode}',
          toastLength: Toast.LENGTH_SHORT,
          gravity: ToastGravity.BOTTOM,
        );
      }
  ...
  Future<void> threesixtyData22() async {
    ...
    try {
      ...
      if (responseInc.statusCode == 200) {
        ...
        final responseTiles = await dio
            ...
            .catchError((error) {
              print("responseurlTiles:$urlTiles");
  ...
  Future<void> threesixtyData() async {
    ...
    try {
      ...
      // Fetch income data with error handling
      final responseInc = await dio.get(
        urlIncome,
        options: Options(
          headers: {"Authorization": 'Bearer $token'},
          validateStatus: (status) {
      ...
      if (responseInc.statusCode == 200) {
        ...
        if (responseData is Map && responseData['status'] == true) {
          ...
          List<String> assetList = ['-Select-'];
          for (var asset in assets) {
            // Ensure asset is a Map and check isArchive condition
            if (asset is Map && asset["isArchive"] != 1) {
              try {
                ...
                if (monthlyRoi != null) {
                  if (monthlyRoi is num) {
                    roiFormatted = monthlyRoi.toStringAsFixed(2);
                  } else {
                    roiFormatted = monthlyRoi.toString();
                  }
                }

                final formattedAsset = "$name ($currency$roiFormatted)";
                assetList.add(formattedAsset);
              } catch (e) {
                print("Error formatting asset: $e");
                // Skip problematic asset formatting
                continue;
              }
            }
          ...
          final incomes = incomeData["incomes"] as List? ?? [];
          final amounts = incomes.map((income) {
            if (income is Map) {
              final amount = income["amount"];
              return amount is num ? amount.round() : 0;
            }
            return 0;
          }).toList();
          // You can use totalIncome if needed, otherwise remove this line
          // final totalIncome = amounts.fold(0, (sum, amount) => sum + amount);
          ...
          // Fetch tiles data with error handling
          final responseTiles = await dio.get(
            urlTiles,
            options: Options(
              headers: {"Authorization": 'Bearer $token'},
              validateStatus: (status) => status! < 500,
          ...
          if (responseTiles.statusCode == 200) {
            ...
            print("tilesData:$tilesData");

            var responseLiability = await dio.get(
              urlLiability,
              options: Options(
                headers: {"Authorization": 'Bearer $token'},
                validateStatus: (status) => status! < 500,
            ...
            print("responseLiability:$responseLiability");
            ...
            if (responseLiability.statusCode == 200) {
              ...
              if (liabilityData is Map) {
                if (liabilityData['status'] == true) {
                  final liabilityContent = liabilityData['data'] as Map? ?? {};
                  final sevengData = liabilityContent["seveng"];

                  // Handle both List and Map cases (though it's always List in your response)
                  ...
                  final sevengData = liabilityData["seveng"];
                  if (sevengData is List) {
                    sevengList = sevengData;
                  } else if (sevengData is Map) {
                    sevengList = [sevengData];
            ...
            Navigator.push(
              context, 
              MaterialPageRoute(
                builder: (context) => Threesixty(data: sevengList),
                maintainState: true,
              ),
            );
          } else {
  ...
  // Helper method to get user-friendly error messages from Dio exceptions
