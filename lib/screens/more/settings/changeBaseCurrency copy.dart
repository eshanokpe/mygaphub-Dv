...
class _CurrencyState extends State<ChangeBaseCurrency> {
  ...
  Future<void> loadCurrencyData() async {
    try {
      ...
      final responsePopularCurrencies = await http.put(
      ...
      if (responsePopularCurrencies.statusCode == 200) {
        ...
        final dataCurrencies =
            body['popular_currencies'] ?? 'Preferences updated';
        print("popular_currencies:$dataCurrencies");
      }
      final jsonData = jsonDecode(data) as List;

      if (mounted) {
        _allCurrencies = jsonData
