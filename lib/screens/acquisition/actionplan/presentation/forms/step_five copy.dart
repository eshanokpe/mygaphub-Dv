...
class _StepFiveState extends ConsumerState<StepFive>
  ...
  // ✅ NEW: builds one Label + Value row (with matching spacing) per field
  // in whichever category the user selected in StepTwo, reading the
  // checklist definition from the shared investment_categories.dart file.
  // Returns an empty list (renders nothing extra) if no category was
  // selected or the category id isn't recognized, so the summary card
  // still shows gracefully instead of throwing.
  ...
  // ✅ NEW: Step 3 and Step 4 store each answer as {"sub_category", "note"}
  // in the same order the questions are asked (see updateStep3Item /
  // updateStep4Item, which always write to a fixed index), so the
  // Summary screen can read them back by position.
