...
class _StepThreeState extends ConsumerState<StepThree> {
  ...
  bool _hasHydratedSavedSavingsGoal = false;

  // ✅ NEW: surfaces a retry UI instead of spinning forever when
  // fetchSeedData()/fetchAnalyticsInfo() throw or just never resolve.
  ...
  void _loadData() async {
    ...
    // If nothing has loaded within 15s, stop spinning silently and show
    // a retry button so "forever" becomes a visible, actionable state.
    _loadTimeoutTimer = Timer(const Duration(seconds: 15), () {
      if (mounted) setState(() => _loadTimedOut = true);
    });

    try {
      await context.read<AuthProvider>().fetchSeedData();
      if (mounted) {
        await context.read<AuthProvider>().fetchAnalyticsInfo(context);
      }
    } catch (e) {
      debugPrint("❌ StepThree data load failed: $e");
  ...
  Widget build(BuildContext context) {
    ...
    // On resume, the strategy data arrives in Riverpod state after this
    // widget has been created. Copy the persisted increase-goal fields into
    // the local text controllers once, without overwriting user edits.
    if (!_hasHydratedSavedSavingsGoal &&
        state.savingsChoice == 'increase_amount' &&
        state.newMonthlySavings.isNotEmpty &&
        state.obtainPlan.isNotEmpty) {
      _hasHydratedSavedSavingsGoal = true;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;

        _newAmountController.value = TextEditingValue(
          text: state.newMonthlySavings,
          selection: TextSelection.collapsed(
            offset: state.newMonthlySavings.length,
          ),
        );
        _explanationController.value = TextEditingValue(
          text: state.obtainPlan,
          selection: TextSelection.collapsed(offset: state.obtainPlan.length),
        );
                        ...
                        : () async {
                            if (_formKey.currentState?.validate() ?? false) {
                              ...
                              controller.updateSavingsGoal(
                                ...
                                newMonthlySavings: newGoalText,
                                obtainPlan: sourceText,
                              );
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        if (_formKey.currentState!.validate()) {
                          final newGoalText = _newAmountController.text.trim();
                          final sourceText = _explanationController.text.trim();

                          // Sync the form fields before the popup invokes
                          // saveAndContinueLater() for Step Three.
                          controller.updateStep3Item(
                            0,
                            "increase_savings",
                            "New Goal: £$newGoalText, Source: $sourceText",
...
class _CurrencyInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    // Keep the input natural: typing 1, 2, 3 produces "123", rather than
    // treating every digit as cents ("0.01", "0.12", "1.23").
    final value = newValue.text.replaceAll(',', '');
    if (RegExp(r'^\d*\.?\d{0,2}$').hasMatch(value)) {
      return newValue.copyWith(
        text: value,
        selection: TextSelection.collapsed(
          offset: newValue.selection.baseOffset.clamp(0, value.length).toInt(),
        ),
      );
    }
