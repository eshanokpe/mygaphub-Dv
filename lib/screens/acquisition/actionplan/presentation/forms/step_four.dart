...
class _StepFourState extends ConsumerState<StepFour> {
  ...
  void initState() {
    ...
    // Seed step4Items with the correct sub_category for every question
    // up front. Without this, any question the user never types into
    // (commonly "other_details", since it's optional) keeps the padded
    // {"sub_category": "", "note": ""} entry from updateStep4Item's
    // list-padding logic in the controller. That blank sub_category gets
    // sent to the backend on save, the request fails, saveAndContinueLater
    // returns false, and Continue silently never advances to StepFive.
            ...
            for (int i = 0; i < _questions.length; i++) ...[
              ...
              TextFormField(
                ...
                validator: (value) {
                  ...
                  if (_questions[i]["sub"] == "other_details") {
                    return null;
                  }
