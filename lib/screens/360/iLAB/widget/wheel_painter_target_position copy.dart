...
class _WheelPainterTargetPositionState
      ...
      switch (liabilitiesType) {
        case 'credit':
          ...
          if (cashLiabilitiesClicked) {
            creditLiability =
                num.tryParse(targetIlab["credit"]?.toString() ?? "0") ?? 0;
          } else {
            creditLiability = 0;
          }
          break;
        case 'mortgage':
          ...
          if (mortageLiabilitiesClicked) {
            mortageLiability =
                num.tryParse(targetIlab["mortgage"]?.toString() ?? "0") ?? 0;
          } else {
            mortageLiability = 0;
          }
          break;
      ...
      switch (assetType) {
        case 'investment':
          ...
          if (investmentAssetClicked) {
            investmentAssets =
                num.tryParse(targetIlab["investment"]?.toString() ?? "0") ?? 0;
          } else {
            investmentAssets = 0;
          }
          break;
        case 'homeEquity':
          ...
          if (homeEquityAssetClicked) {
            equityAssets =
                num.tryParse(targetIlab["equity"]?.toString() ?? "0") ?? 0;
          } else {
            equityAssets = 0;
          }
          break;
        case 'cash':
          ...
          if (cashAssetClicked) {
            savingsAssets =
                num.tryParse(targetIlab["savings"]?.toString() ?? "0") ?? 0;
          } else {
            savingsAssets = 0;
          }
          break;
      ...
      switch (incomeType) {
        case 'non-portfolio':
          ...
          if (nonPortfolioIncomeClicked) {
            clickedNonPortfolio =
                num.tryParse(targetIlab["non_portfolio"]?.toString() ?? "0") ??
                0;
          } else {
        ...
        case 'portfolio':
          ...
          if (portfolioIncomeClicked) {
            clickedPortfolio =
                num.tryParse(
                  targetIlab["asset_portfolio"]?.toString() ?? "0",
                ) ??
                0;
          } else {
            clickedPortfolio = 0;
      ...
      switch (budgetType) {
        case 'periodic':
          ...
          if (periodicClicked) {
            clickedPeriodic =
                num.tryParse(
                  targetIlab["periodic_savings"]?.toString() ?? "0",
                ) ??
                0;
          } else {
            clickedPeriodic = 0;
        ...
        case 'education':
          ...
          if (educationClicked) {
            clickedEducation =
                num.tryParse(targetIlab["education"]?.toString() ?? "0") ?? 0;
          } else {
            clickedEducation = 0;
          }
          break;
        case 'expenditure':
          ...
          if (expenditureClicked) {
            clickedExpenditure =
                num.tryParse(targetIlab["expenditure"]?.toString() ?? "0") ?? 0;
          } else {
        ...
        case 'discretionary':
          ...
          if (discretionaryClicked) {
            clickedDiscretionary =
                num.tryParse(targetIlab["discretionary"]?.toString() ?? "0") ??
                0;
          } else {
  ...
  Widget build(BuildContext context) {
    ...
    var investment1 = invTick1
        ? (num.tryParse(targetIlab["investment"]?.toString() ?? "0") ?? 0)
        : 0;
    var equity1 = equTick1
        ? (num.tryParse(targetIlab["equity"]?.toString() ?? "0") ?? 0)
        : 0;
    var savings1 = savTick1
        ? (num.tryParse(targetIlab["savings"]?.toString() ?? "0") ?? 0)
        : 0;

    // Calculate asset total with clicked amounts subtracted
    num assetTotal1 =
        (investment1 - investmentAssets) +
        (equity1 - equityAssets) +
        (savings1 - savingsAssets);
    ...
    var nonP0 = npTick0
        ? (num.tryParse(targetIlab["non_portfolio"]?.toString() ?? "0") ?? 0)
        : 0;
    var port0 = portTick0
        ? (num.tryParse(targetIlab["asset_portfolio"]?.toString() ?? "0") ?? 0)
        : 0;
    ...
    var periodic0 = perTick0
        ? (num.tryParse(targetIlab["periodic_savings"]?.toString() ?? "0") ?? 0)
        : 0;
    ...
    var discretionary0 = discTick0
        ? (num.tryParse(targetIlab["discretionary"]?.toString() ?? "0") ?? 0)
        : 0;
    ...
    return GestureDetector(
      onTapUp: (details) {
        ...
        if (distanceFromCenter > 80) {
          ...
          if (dx > 0 && dy < 0) {
            index = 1; // Purple - Liabilities
            print('Purple segment (Liabilities)');
          } else if (dx > 0 && dy > 0) {
            index = 2; // Red - Budget
            print('Red segment (Budget)');
          } else if (dx < 0 && dy > 0) {
            index = 3; // Orange - Income
            print('Orange segment (Income)');
          } else {
            index = 0; // Green - Assets
            print('Green segment (Assets)');
          }
                  ...
                  Positioned(
                    ...
                    child: GestureDetector(
                      onTap: () {
                        ...
                        setState(() {
                          selectedIndex = -1;
                          // Reset all transfers when closing the segment
  ...
  Widget _centerTileBudget({
    ...
    num periodicValue =
        num.tryParse(targetIlab["periodic_saving"]?.toString() ?? "0") ?? 0;
    num educationValue =
        num.tryParse(targetIlab["education"]?.toString() ?? "0") ?? 0;
  ...
  // Professional position calculation
