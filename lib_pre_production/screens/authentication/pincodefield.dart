...
class _PinCodeTextFieldState extends State<PinCodeTextField>
  ...
  void _checkForInvalidValues() {
    ...
    assert(widget.fieldHeight > 0);
    assert(widget.fieldWidth > 0);
    assert(widget.borderWidth >= 0);
    assert(widget.affirmativeText.isNotEmpty);
    assert(widget.negativeText.isNotEmpty);
    assert(widget.dialogTitle.isNotEmpty);
    assert(widget.dialogContent.isNotEmpty);
  }
  ...
  Future<void> _showPasteDialog(String pastedText) {
    var formattedPastedText = pastedText.trim().substring(
      0,
      min(pastedText.trim().length, widget.length),
    );
    return showDialog(
      context: context,
      builder: (context) {
