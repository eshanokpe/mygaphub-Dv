...
class ViewHistoricReportChart extends StatefulWidget {
  ...
  @override
  State<ViewHistoricReportChart> createState() =>
      _ViewHistoricReportChartState();
}

class _ViewHistoricReportChartState extends State<ViewHistoricReportChart> {
  Map dashData = {};
  Map seedData = {};
  Dio dio = Dio();
  Map historicData = {};
  ...
  List<charts.Series<dynamic, String>> _createSeriesList() {
    ...
    for (var entry in widget.data!.keys) {
      ...
      actualData.add(
        ChartData(
          period,
          int.tryParse(widget.data![entry]['actual'].toString()) ?? 0,
        ),
      );
